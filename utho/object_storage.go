...
// 	reqUrl := "objectstorage/" + dcslug + "/accesskey/" + accesskeyName + "/delete"
// 	req, _ := s.client.NewRequest("DELETE", reqUrl)
...
type CreateDirectroyParams struct {
	Dcslug     string
	BucketName string
	Path       string `json:"path"` //Expample:- /name
}
...
type UpdateBucketAccessKeyPermissionParams struct {
	Dcslug         string
	BucketName     string
	PermissionName string `json:"selected_permission"`
	AccessKeyId    string `json:"selected_key"`
}
